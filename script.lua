...
local function SwitchCategory(catName)
    currentCat = catName; local targetBtn = catButtons[catName]; local t = Themes[Cfg.UITheme] or Themes.Dark
    if targetBtn then
...
local function ToggleMenu()
    MenuOpen = not MenuOpen; local t = Themes[Cfg.UITheme] or Themes.Dark
    if MenuOpen then 
...
local function CreateModule(name, key, category)
    ...
    local Toggle = Instance.new("TextButton", ModFrame); Toggle.Size = UDim2.new(0, 45, 0, 22); Toggle.Position = UDim2.new(1, -55, 0, 7); Toggle.BackgroundColor3 = Cfg[key] and Color3.new(0, 0.8, 0) or Color3.new(0.8, 0, 0); Toggle.Text = ""; Instance.new("UICorner", Toggle).CornerRadius = UDim.new(1,0)
...
local function GetTarget()
    local t, d = nil, Cfg.AimbotMaxDistance
...
local function GetKillauraTarget()
    local myChar = LocalPlayer.Character; if not myChar or not myChar:FindFirstChild("HumanoidRootPart") then return nil end
    local myPos = myChar.HumanoidRootPart.Position
    local t, d = nil, Cfg.KillAuraRange
    for _, v in ipairs(Players:GetPlayers()) do
        if v ~= LocalPlayer and not FriendsList[LowerNameCache[v.Name]] and v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health > 0 then
            local dist = (myPos - v.Character.HumanoidRootPart.Position).Magnitude
...
local function CreateStar(position)
    local bgui = Instance.new("BillboardGui", GeminiGui); bgui.Size = UDim2.new(Cfg.ParticleSize*0.5,0,Cfg.ParticleSize*0.5,0); bgui.AlwaysOnTop = true
    ...
    local f = Instance.new("TextLabel", bgui); f.Size = UDim2.new(1,0,1,0); f.BackgroundTransparency = 1; f.Text = "★"; f.TextScaled = true; f.TextColor3 = Cfg.ParticleColor; f.Font = Enum.Font.GothamBlack
...
local function CreateJumpCircle(pos)
    if not Cfg.JumpVisualCirclesEnabled then return end
    local p = Instance.new("Part", workspace); p.Shape = Enum.PartType.Cylinder; p.Size = Vector3.new(0.1, 0, 0); p.CFrame = CFrame.new(pos - Vector3.new(0, 2.9, 0)) * CFrame.Angles(0, 0, math.rad(90)); p.Transparency = 0.5; p.Anchored = true; p.CanCollide = false; p.Material = Enum.Material.Neon; p.Color = Cfg.JumpCircleEffectColor
...
table.insert(Connections, RunService.Stepped:Connect(function(time, dt)
    ...
    if Cfg.AnimLagEnabled then
        local hum = char:FindFirstChild("Humanoid")
        if hum then
            local animator = hum:FindFirstChild("Animator")
            if animator then
                AnimLagAccumulator = AnimLagAccumulator + dt
                local interval = 1 / math.clamp(tonumber(Cfg.AnimLagFPS) or 5, 1, 60)
                    ...
                    if player ~= LocalPlayer then
                        ...
                        if plChar then
                            ...
                            if head and head:IsA("BasePart") then
                                ...
                                local isHitboxActive = not isFriend and (not Cfg.HitboxOnlyKillaura or player == SharedKaTarget)
                                local mult = tonumber(Cfg.HitboxSize) or 1
                                
                                if not OrigPartData[head] then OrigPartData[head] = {Size = head.Size, Trans = head.Transparency, CanCollide = head.CanCollide, Massless = head.Massless} end
                                
                                if isHitboxActive then
                                    local targetSize = OrigPartData[head].Size * mult
                                    if head.Size ~= targetSize then
                                        head.Size = targetSize; head.Transparency = 1; head.CanCollide = false; head.Massless = true
                                        local face = head:FindFirstChildOfClass("Decal"); if face then if not OrigPartData[face] then OrigPartData[face] = {Trans = face.Transparency} end; face.Transparency = 1 end
                                        
                                        if not plChar:FindFirstChild("Gemini_FakeHead") then
                                            local fakeHead = head:Clone(); fakeHead.Name = "Gemini_FakeHead"
                                            for _, v in ipairs(fakeHead:GetChildren()) do if v:IsA("Attachment") or v:IsA("Weld") or v:IsA("WeldConstraint") or v:IsA("Motor6D") or v:IsA("Script") or v:IsA("LocalScript") then v:Destroy() end end
                                            fakeHead.Size = OrigPartData[head].Size; fakeHead.Transparency = OrigPartData[head].Trans; fakeHead.CanCollide = false; fakeHead.Massless = true
                                            local fakeFace = fakeHead:FindFirstChildOfClass("Decal"); if fakeFace and OrigPartData[face] then fakeFace.Transparency = OrigPartData[face].Trans end
                                            local w = Instance.new("WeldConstraint"); w.Part0 = head; w.Part1 = fakeHead; w.Parent = fakeHead; fakeHead.Parent = plChar
                                        end
                                    end
                                else
                                    if head.Size ~= OrigPartData[head].Size then
                                        ...
                                        local fakeH = plChar:FindFirstChild("Gemini_FakeHead"); if fakeH then fakeH:Destroy() end
                                    end
                                end
                            end
                        end
...
table.insert(Connections, RunService.RenderStepped:Connect(function(dt)
    ...
    if char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChild("Humanoid") then
        ...
        if Cfg.AirStrafeEnabled and isAirborne and hum.MoveDirection.Magnitude > 0 then
            ...
            local targetVel = hum.MoveDirection.Unit * speed; hrp.AssemblyLinearVelocity = Vector3.new(targetVel.X, currentVel.Y, targetVel.Z)
        end
        
        if Cfg.VelocityEnabled then
            local hMult = tonumber(Cfg.VelocityHorizontal) or 0; local vMult = tonumber(Cfg.VelocityVertical) or 0
            if hMult == 0 then if hum.MoveDirection.Magnitude == 0 then hrp.AssemblyLinearVelocity = Vector3.new(0, hrp.AssemblyLinearVelocity.Y, 0) elseif not Cfg.StrafeEnabled and not (Cfg.AirStrafeEnabled and isAirborne) then local currentSpeed = Cfg.SpeedEnabled and Cfg.WalkSpeedValue or hum.WalkSpeed; local targetVel = hum.MoveDirection.Unit * currentSpeed; hrp.AssemblyLinearVelocity = Vector3.new(targetVel.X, hrp.AssemblyLinearVelocity.Y, targetVel.Z) end end
        ...
        if player ~= LocalPlayer then
            ...
            if Cfg.Esp2DBoxEnabled and hasHRP and not isFriend and (not Cfg.Esp2DBoxTeamCheck or not sameTeam) and not isIgnoredTeam then
                ...
                if onScreen then
                    ...
                    local height = math.abs(bottomScreen.Y - topScreen.Y); local baseMultiplier = tonumber(Cfg.Esp2DBoxSize) or 1; local finalHeight = height * baseMultiplier; local width = (height / 1.5) * baseMultiplier 
    ...
    if Cfg.ChinaHatAccessoryEnabled and char and char:FindFirstChild("Head") then HatPart.Transparency = Cfg.ChinaHatTransparency; HatPart.Color = Cfg.ChinaHatAccessoryColor; HatMesh.Scale = Vector3.new(Cfg.ChinaHatWidthScale, Cfg.ChinaHatHeightScale, Cfg.ChinaHatWidthScale); HatPart.CFrame = char.Head.CFrame * CFrame.new(0, Cfg.ChinaHatHeightOffset, 0) else HatPart.Transparency = 1 end

    if Cfg.TargetHudEnabled and hudTarget and hudTarget.Character and hudTarget.Character:FindFirstChild("Humanoid") then
        if not TargetHUD.Visible then TargetHUD.Visible = true end; local hum = hudTarget.Character.Humanoid
    ...
    if Cfg.TargetESPSquareEnabled and espTarget and espTarget.Character and espTarget.Character:FindFirstChild("HumanoidRootPart") then
        ...
        local currentEspColor = Cfg.TargetESPSquareColor
        if Cfg.TargetESPDamageColorEnabled then local timeSinceDmgEsp = tick() - lastDamageTimeESP; if timeSinceDmgEsp < 0.6 then currentEspColor = Cfg.TargetESPDamageColor:Lerp(Cfg.TargetESPSquareColor, timeSinceDmgEsp / 0.6) end end
        local pos, onScreen = Camera:WorldToViewportPoint(espTarget.Character.HumanoidRootPart.Position)
        if onScreen then
            if not ESPMain.Visible then ESPMain.Visible = true end; ESPMain.Position = UDim2.new(0, pos.X, 0, pos.Y); ESPMain.Size = UDim2.new(0, Cfg.TargetESPSquareSize, 0, Cfg.TargetESPSquareSize); ESPMain.Rotation = (tick() * 60 * Cfg.TargetESPRotationSpeed) % 360 
            local updateThickness = false; if lastRenderedEspThickness ~= Cfg.TargetESPBorderThickness then updateThickness = true; lastRenderedEspThickness = Cfg.TargetESPBorderThickness end
    ...
    if Cfg.KillAuraEnabled and char and localHrp and currentKaTarget and currentKaTarget.Character then
        local targetPart = currentKaTarget.Character:FindFirstChild("HumanoidRootPart")
        if targetPart then
            local hrp = localHrp; local dist = (hrp.Position - targetPart.Position).Magnitude
            if dist <= Cfg.KillAuraRange and IsVisible(targetPart) then
                if Cfg.KillStrafeEnabled and char:FindFirstChild("Humanoid") then
                    ...
                    if distFlat > 0.1 then
                        ...
                        local dirToTarget = flatToTarget.Unit; local rightDir = dirToTarget:Cross(Vector3.new(0, 1, 0)).Unit * currentKaStrafeDir; local noise = math.sin(tick() * 4) * 0.3; local currentOrbitDist = tonumber(Cfg.KillStrafeDistance) or 1; local distanceError = distFlat - (currentOrbitDist + noise); local moveDir = (dirToTarget * distanceError + rightDir * 3).Unit
                        char.Humanoid:Move(moveDir, false); local kStrafeSpeed = tonumber(Cfg.KillStrafeSpeed) or 20; hrp.AssemblyLinearVelocity = Vector3.new(moveDir.X * kStrafeSpeed, hrp.AssemblyLinearVelocity.Y, moveDir.Z * kStrafeSpeed)
                        local shouldJump = true; if Cfg.CriticalsNoKillStrafeJump then shouldJump = false end
                ...
                if dist <= (Cfg.KillAuraClickRange or 15) then
                    local attackDelay = (Cfg.KillAuraSpeed / 10)
                    if tick() - lastAttackTime > attackDelay then
                        ...
                        if Cfg.HitSoundEnabled then local sIdx = math.clamp(math.floor(Cfg.HitSoundMode), 1, 6); local s = Instance.new("Sound", game:GetService("SoundService")); s.SoundId = HitSounds[sIdx]; s.Volume = 2; s:Play(); game:GetService("Debris"):AddItem(s, 1) end
                        if Cfg.DamageParticlesEnabled then local pAmt = tonumber(Cfg.ParticleAmount) or 8; for i = 1, pAmt do CreateStar(targetPart.Position) end end
                    end
                end
            end
        end
    ...
    if not didRotateHRP and Cfg.JitterEnabled and char and char:FindFirstChild("Humanoid") and localHrp then
        local hrp = localHrp
        char.Humanoid.AutoRotate = false
        
        local state = char.Humanoid:GetState()
        if Cfg.JitterSpinAtJump and (state == Enum.HumanoidStateType.Freefall or state == Enum.HumanoidStateType.Jumping) then 
            local spinSpeed = tonumber(Cfg.JitterSpinSpeed) or 20
            local spinAngle = math.rad((tick() * spinSpeed * 15) % 360)
            hrp.CFrame = CFrame.new(hrp.Position) * CFrame.Angles(0, spinAngle, 0)
        else 
            local jitterSign = (math.floor(tick() * (Cfg.JitterSpeed or 15)) % 2 == 0) and 1 or -1
            local jitterYaw = math.rad(Cfg.JitterRange or 45) * jitterSign
            local baseYaw = (tonumber(Cfg.JitterYawMode) == 2) and math.pi or 0
            local totalYaw = baseYaw + jitterYaw 
            
            local lookVec = camCFrame.LookVector; if char.Humanoid.MoveDirection.Magnitude > 0 then lookVec = char.Humanoid.MoveDirection end
    ...
    if Cfg.CriticalsEnabled and char and localHrp and char:FindFirstChild("Humanoid") then
        local canCrit = true; if Cfg.CriticalsOnlyKillAura and not Cfg.KillAuraEnabled then canCrit = false end
        if canCrit and char.Humanoid.FloorMaterial ~= Enum.Material.Air then
            localHrp.CFrame = localHrp.CFrame + Vector3.new(0, 1, 0); localHrp.AssemblyLinearVelocity = Vector3.new(localHrp.AssemblyLinearVelocity.X, 0, localHrp.AssemblyLinearVelocity.Z); lastCriticalJumpTime = tick()
        end
    end

    if not Cfg.KillAuraEnabled and Cfg.AimbotEnabled and target and target.Character and target.Character:FindFirstChild("Head") and localHrp then
        Camera.CFrame = camCFrame:Lerp(CFrame.new(camPos, target.Character.Head.Position), Cfg.AimbotSmoothness)
    end
end))
...
local function UpdateMobileBinds()
    ...
    for key, bindLetter in pairs(activeModules) do
        ...
        if not btn then
            ...
            local savedPos = Cfg["MobilePos_"..key]; if savedPos then btn.Position = UDim2.new(savedPos.XScale, savedPos.XOffset, savedPos.YScale, savedPos.YOffset) else btn.Position = UDim2.new(0.8, math.random(-50, 50), 0.5, math.random(-50, 50)) end
            btn.BackgroundColor3 = Cfg[key] and Color3.fromRGB(0, 180, 0) or t.inputBg; btn.TextColor3 = t.text; btn.Font = TARGET_FONT; btn.TextSize = 22; btn.Text = bindLetter; Instance.new("UICorner", btn).CornerRadius = UDim.new(1, 0)
...
do
    ...
    local function RefreshFriends()
        local th = Themes[Cfg.UITheme] or Themes.Dark
    ...
    UI.tBtn = Instance.new("TextButton", UI.tC); UI.tBtn.Size = UDim2.new(1, -20, 0, 40); UI.tBtn.Position = UDim2.new(0, 10, 0, 35); UI.tBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 40); UI.tBtn.TextColor3 = Color3.new(1,1,1); UI.tBtn.Text = "Theme: " .. tostring(Cfg.UITheme); UI.tBtn.Font = TARGET_FONT; UI.tBtn.TextSize = 14; Instance.new("UICorner", UI.tBtn).CornerRadius = UDim.new(0,4); table.insert(ThemeObjects.InputBackgrounds, UI.tBtn); table.insert(ThemeObjects.Texts, UI.tBtn)
